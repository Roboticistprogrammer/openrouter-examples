...
def main() -> None:
    ...
    name = os.getenv("OPENROUTER_KEY_NAME", "Example key from SDK")
    expires_at_raw = os.getenv("OPENROUTER_KEY_EXPIRES_AT", "")

    create_kwargs: dict[str, object] = {"name": name}
    if expires_at_raw:
        create_kwargs["expires_at"] = parse_datetime(expires_at_raw)
    ...
    with OpenRouter(
        ...
        res = client.api_keys.create(**create_kwargs)
        print(res)
